...
to `cache_path`. If `cache_path` is not set, bedbaser will choose the
default location.
...
bedbase <- BEDbase()
```
bedbaser can use the same cache as [geniml](https://docs.bedbase.org/geniml/)'s
BBClient by setting the `cache_path` to the same location. It will create
